...
> 📦 **Adapted from Eco-Flow's [Nextflow training course](https://github.com/Eco-Flow/training).** This page is included here as reading material for the concepts and commands. The hands-on Slurm setup (`hpc/start_slurm.sh`, `hpc/slurm_codespaces.config`, `hpc/run_demo_slurm.sh`) referenced below lives in the original course repo's devcontainer and isn't wired up in this repo — read alongside the original if you want to actually run the Slurm steps, or adapt them to your own environment.
...
`nextflow list` shows every pipeline Nextflow has downloaded for you, and `nextflow info` shows where it keeps them:
...
With a clone, you point Nextflow at the **folder** rather than the pipeline name. There's nothing to run here: Step 4 uses the copy from Way A.
...
> ⚠️ **Always pin the version** (see [Running an nf-core ampliseq pipeline](../01-Ampliseq/README.md) for why). Nextflow itself changes too, and a very old pipeline release may not run on a current Nextflow. If an old release stops with `Config parsing failed`, move `-r` to a newer release.
...
Keep the clone — you'll run it in the next step. For now, go back to your working folder, so the run's working files land there.
...
>
> Nextflow writes a job script for every task: the `.command.run` file. Take the hash from a **FASTQC** line of *your* output and look at the top of its script:
>
> ```bash
...
#SBATCH -o .../work/94/ace98a.../.command.log
#SBATCH --no-requeue
...
You rarely write this yourself: if your cluster is listed on [nf-co.re/configs](https://nf-co.re/configs), its profile already contains it (Step 6). Writing one from scratch is covered in the advanced HPC config guide (not included in this workshop repo — see the original Eco-Flow training course).
</details>
...
<img src="img/nfcore_configs_list.png" alt="The nf-co.re/configs page: a searchable list of institutional configs, each row showing a cluster description and the executor it uses, such as slurm" width="650"/>
...
**Open your cluster's page first** — most list setup steps, such as which Java module to load.
...
Writing one yourself is covered in the advanced HPC config guide (not included in this workshop repo — see the original Eco-Flow training course).
...
<details markdown="1">
<summary>🧬 Optional — rerun an nf-core/rnaseq analysis on your cluster</summary>

Eco-Flow's training course walks through an nf-core/rnaseq analysis with its own sample data, which you can repeat on a real cluster. Clone that repo directly:

```bash
git clone https://github.com/Eco-Flow/training.git
cd training/eco-flow-training

# The samplesheet, with paths for wherever you cloned it
...
- Stuck? The [nf-core Slack](https://nf-co.re/join/slack) is full of people running pipelines on clusters like yours.
